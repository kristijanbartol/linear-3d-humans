...
class STAR(nn.Module):
    def __init__(self,gender='female',num_betas=10):
        ...
        if gender == 'male':
            path_model = '/media/kristijan/kristijan-hdd-ex/anthropometry-baselines/models/star/STAR_MALE.npz'
        elif gender == 'female':
            path_model = '/media/kristijan/kristijan-hdd-ex/anthropometry-baselines/models/star/STAR_FEMALE.npz'
        else:
            path_model = '/media/kristijan/kristijan-hdd-ex/anthropometry-baselines/models/star/STAR_NEUTRAL.npz'
        ...
        self.register_buffer('J_regressor', torch.cuda.FloatTensor(J_regressor).to('cpu'))

        # Model skinning weights
        #self.register_buffer('weights', torch.cuda.FloatTensor(star_model['weights']))
        self.register_buffer('weights', torch.cuda.FloatTensor(star_model['weights']).to('cpu'))
        # Model pose corrective blend shapes
        self.register_buffer('posedirs', torch.cuda.FloatTensor(star_model['posedirs'].reshape((-1,93))).to('cpu'))
        # Mean Shape
        self.register_buffer('v_template', torch.cuda.FloatTensor(star_model['v_template']).to('cpu'))
        # Shape corrective blend shapes
        self.register_buffer('shapedirs', torch.cuda.FloatTensor(np.array(star_model['shapedirs'][:,:,:num_betas])).to('cpu'))
        # Mesh traingles
        self.register_buffer('faces', torch.from_numpy(star_model['f'].astype(np.int64)).to('cpu'))
        self.f = star_model['f']
        # Kinematic tree of the model
        self.register_buffer('kintree_table', torch.from_numpy(star_model['kintree_table'].astype(np.int64)).to('cpu'))
